...
// uniform vec3 sh_coeffs[10];

vec3 calc_sh(vec3 normal) {
	vec3 tnorm   = normalize(nmat * normal);
	return vec3(0.7, 1.0, 0.7) * (C1 * L22 * (tnorm.x * tnorm.x - tnorm.y * tnorm.y) +
...
void main() {
	...
	norm = rotmat * norm;
	v_ambient = calc_sh(norm)*0.25;
	float diff = dot(norm, nmat * lightdir);
	diff = max(diff*1.75-1.0, 0.0);
	v_diffuse = vec3(diff);
